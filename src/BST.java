...
// class binary search tree, you can find the comments for the main methods in the interface (BSTMethods).
public class BST<T> implements BSTMethods<T>{

	public BSTNode<T> root, current; // nodes.
	...
	private void preOrderTraversal(BSTNode<T> node) {
	    if (node != null) {
	    	if(node.getData() instanceof Word) {
	    		Word w= (Word)node.getData();
	    		System.out.println("\n========");
